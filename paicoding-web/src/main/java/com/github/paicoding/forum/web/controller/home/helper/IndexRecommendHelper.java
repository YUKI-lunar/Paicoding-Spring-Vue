...
public class IndexRecommendHelper {
     ...
     * 置顶top 文章列表
     */
    //todo 这里我把返回逻辑改成了读取cover,没有就用默认,需要完成:1.默认图片设置   2.前端上传cover的部分
    public List<ArticleDTO> topArticleList(CategoryDTO category){
        List<ArticleDTO> topArticles = articleService.queryTopArticlesByCategory(category.getCategoryId());
        topArticles.forEach(s -> {
            if(s.getCover() == null){
                s.setCover(CommonConstants.DEFUALT_IMAGE);
            }
        });
        return topArticles;
    }
