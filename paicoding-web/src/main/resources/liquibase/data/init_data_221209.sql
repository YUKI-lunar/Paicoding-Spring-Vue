...
VALUES(1, 1, '管理员', 'https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/images/avatar/B0FBE96A0309EF08D329E782434894E9.jpg', 'java', 'xm', '码农', '', 0);
...
VALUES(1, '小灰飞的专栏', 1, '这里是小灰飞的踩坑记录，欢迎关注', 'https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/images/cover/column-avatar1.png', 1, '2022-09-15 01:00:00', '2022-09-15 01:00:00', '2022-09-15 01:00:00');
...
JVM 全称是 Java Virtual Machine，中文译名Java虚拟机。JVM 本质上是一个运行在计算机上的程序，他的职责是运行Java字节码文件。
Java源代码执行流程如下：
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/378a97d805347c7a94f4e8955d7b933a.png)
分为三个步骤：
1、编写Java源代码文件。
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/2a82bb72c6868bc1a6af3c11257b946c.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/56579da5307f70965964599e82251a7c.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/a14f500a7d347cc06544bbf98f1c5330.png)

但是C/C++语言，如果要让程序在不同平台上运行，就需要将一份源代码在不同平台上分别进行编译，相对来说比较麻烦。
再回到即时编译，在JDK1.1的版本中就推出了即时编译去优化对应的性能。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/b21254144e39d8a32784b6c6b4bb8f8d.png)
...
Java虚拟机主要分为以下几个组成部分：
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/e2e15cec89b785c02b8d465e722aab7c.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/8bf944342c3180567a595506c6785602.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/7e119baa0eaf0bedced10d76323f7784.png)

无法解读出文件里包含的内容，推荐使用 jclasslib工具查看字节码文件。 Github地址： https://github.com/ingokegel/jclasslib

![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/717347ff910bd7744087566ebeec4465.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/63b3035c8c2cd03be9fe696066d9a9e2.png)
...
比如常见的文件格式校验方式如下：
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/2096d15baba88e02c2606fda429ce715.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/f78bd566b2c88815ab19253e560e2817.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/88da89e7709e87bd76b7540e0030251a.png)


#### 3.2.2 常量池

字节码文件中常量池的作用：避免相同的内容重复定义，节省空间。如下图，常量池中定义了一个字符串，字符串的字面量值为123。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/2914192760d83b9d86f6f1bf9a9d79ad.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/182824136eec5204c355730a80edefb5.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/afe53c01bc789b38da5dfd5605863860.png)


#### 3.2.4 方法

字节码中的方法区域是存放**字节码指令**的核心位置，字节码指令的内容存放在方法的Code属性中。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/fba1504737081e51f24beb41f267579e.png)
...
这段代码编译成字节码指令之后是如下内容：
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/000d65dd9aef2b3af9e1124a5de5c3fb.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/eefd483b3fb4c0dd4c82d163bb1551c1.png)

2、istore_1会从操作数栈中，将栈顶的元素弹出来，此时0会被弹出，放入局部变量表的1号位置。局部变量表中的1号位置，在编译时就已经确定是局部变量i使用的位置。完成了对局部变量i的赋值操作。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/a44f6aa0a6c6b863c9e7c5cdb5a96584.png)

3、iload_1将局部变量表1号位置的数据放入操作数栈中，此时栈中会放入0。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/778dd2dd513630fb7ec3e68fb4f08b2a.png)

4、iconst_1会将常量1放入操作数栈中。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/0fb353711da21cd4122ab18aa2dc463b.png)

5、iadd会将操作数栈顶部的两个数据相加，现在操作数栈上有两个数0和1，相加之后结果为1放入操作数栈中，此时栈上只有一个数也就是相加的结果1。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/78bc101711b04573516cbb74b759b2c2.png)

6、istore_2从操作数栈中将1弹出，并放入局部变量表的2号位置，2号位置是j在使用。完成了对局部变量j的赋值操作。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/f4eb3313b48d00a118adb3cfcad64386.png)

7、return语句执行，方法结束并返回。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/d941f189636fd844212be8429a733ccf.png)

同理，同学们可以自行分析下i++和++i的字节码指令执行的步骤。
i++的字节码指令如下，其中iinc 1 by 1指令指的是将局部变量表1号位置增加1，其实就实现了i++的操作。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/3eabbe2fb68d584a147916714d4a9b33.png)

而++i只是对两个字节码指令的顺序进行了更改：
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/24d6938804c52a1928800229d792cbcd.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/da80d7543580cc80f7952cf6708e507f.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/1310c6b7b98e7e43101f586476d693ab.png)
...
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/9d85b83fceb86113b11dd56fd439fa76.png)

**安装方法：**
1、使用java -jar arthas-boot.jar  启动程序。
2、输入需要Arthas监控的进程id。
![](https://-oss.oss-cn-beijing.aliyuncs.com/tech-pai/0725e762274911d14b5a5de407a496eb.png)
